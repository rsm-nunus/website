...
sns.histplot(
    ...
    bins=50,
    log_scale=(False, True),    # log y-axis for visibility
    color="#1f77b4"
)
plt.xlabel("Number of Reviews")
plt.title("Distribution of Review Counts (log y-scale)")
plt.tight_layout()
...
# ▸ Remove zero-review listings OR add 1 to every count
data = airbnb["number_of_reviews"]
data_pos = data[data > 0]          # keep strictly positive counts
...
plt.yscale("log")                  # set log scale after plotting
plt.xlabel("Number of Reviews (positive counts only)")
plt.title("Distribution of Review Counts (log y-scale)")
plt.tight_layout()

sns.histplot(
    data + 1,                      # add 1 to every observation
    bins=50,
    color="#1f77b4",
    edgecolor="white"
)
plt.yscale("log")
plt.xlabel("Number of Reviews (+1 shift)")
